...
apiClient.interceptors.request.use(config => {
  if (accessToken && config.url !== '/api/v1/auth/refresh') config.headers.Authorization = `Bearer ${accessToken}`
  return config
})

apiClient.interceptors.response.use(response => response, async (error: AxiosError) => {
  const original = error.config as (InternalAxiosRequestConfig & { _retried?: boolean }) | undefined
  if (!original || error.response?.status !== 401 || original._retried || original.url === '/api/v1/auth/refresh') throw error
  original._retried = true
  refreshPromise ??= apiClient.post('/api/v1/auth/refresh').then(response => {
