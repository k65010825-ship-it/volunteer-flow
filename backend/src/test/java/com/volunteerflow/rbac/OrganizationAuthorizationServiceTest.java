...
class OrganizationAuthorizationServiceTest {
    ...
    void protectedOwnerHasEveryCurrentAndFuturePermission() {
        ...
        assertThatCode(() -> service.requirePermission(1L, 100L, "future:permission")).doesNotThrowAnyException();
    }
    ...
    void ordinaryMemberWithoutPermissionIsForbidden() {
        ...
        assertThatThrownBy(() -> service.requirePermission(1L, 100L, "activity:publish"))
                ...
                .extracting(ex -> ((BusinessException) ex).status().value()).isEqualTo(403);
    }

    private OrganizationMember member(Long roleId) {
        OrganizationMember member = new OrganizationMember();
        member.setOrganizationId(100L); member.setUserId(1L); member.setRoleId(roleId); member.setStatus("ACTIVE");
        return member;
    }

    private RbacRole role(Long id, String type, boolean protectedRole) {
        RbacRole role = new RbacRole();
        role.setId(id); role.setOrganizationId(100L); role.setBuiltInType(type); role.setProtectedRole(protectedRole); role.setStatus("ACTIVE");
        return role;
    }
}
