...
class Stage1VmAcceptanceIT {
    ...
    void completesStageOneBusinessLoopAgainstConfiguredMysql() {
        ...
        Organization organization = organizations.create(platformAdmin.getId(), platformAdmin.getPlatformRole(),
                new OrganizationService.CreateOrganizationRequest("阶段一事务验收", "自动回滚", owner.getId()));
        InviteCode invite = organizations.createInvite(owner.getId(), organization.getId(),
        ...
        var activity = activities.create(owner.getId(), organization.getId(), new ActivityService.ActivityRequest(
                "新生开学典礼", "阶段一多岗位活动", "图书馆前广场",
                base, base.plusHours(2), base.plusDays(1), base.plusDays(2), base.plusDays(2).plusHours(4)));
        activities.addPosition(owner.getId(), activity.getId(), new ActivityService.PositionRequest(
                "礼仪岗", "负责接待引导", 20, "FIRST_COME", 120, null, null, null));
        activities.addPosition(owner.getId(), activity.getId(), new ActivityService.PositionRequest(
                "讲解岗", "负责校园讲解", 15, "REVIEW", 120, null, null, null));
